...
function switchSysBar(){
     if (1 == window.status){
		  window.status = 0;
         switchPoint.innerHTML = '打开侧栏';//<img src="skins/images/left.gif" title="关闭/打开左栏">
          document.all("leftmain").style.display="none"
     }
     else{
		  window.status = 1;
          switchPoint.innerHTML = '关闭侧栏';//<img src="skins/images/right.gif" title="关闭/打开左栏">';
          document.all("leftmain").style.display=""
     }
}

function openmenu(){
	var objtitle=document.getElementById("leftmenu_title");
	var leftobj;
	leftobj = document.all ? frames["frmleft"] : document.getElementById("frmleft").contentWindow;
	if (!leftobj){return;}
	var menubar = leftobj.document.getElementById("menubar")
	
	objtitle.innerHTML='系统管理';
	menubar.innerHTML='';
}
